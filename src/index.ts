...
type Bindings = {
    ...
    MAILER_URI: string // Thêm biến môi trường cho Mailer
}
...
app.get(
    '/',
    cache({ cacheName: 'otp-api', cacheControl: 'max-age=60' }),
    async (c) => {
        return c.json({
...
app.post('/api/otp/generate', validateRequest(generateOtpSchema), async (c) => {
    try {
        const data = generateOtpSchema.parse(c.get('validatedData')) // Kiểm tra lại dữ liệu đã được xác thực
